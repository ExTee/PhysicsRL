...
public class LCPApp implements SceneGraphNode, Interactor {
    ...
    private TwoBlocks twoBlocks = new TwoBlocks(system);
    
    private PinkBlocks pinkBlocks = new PinkBlocks(system);
    ...
    public LCPApp() {
        ...
        //loadPinkBallsSystem("datalcp/pinkballs.png");
        //loadPinkBallsSystem("datalcp/AdvancedPinky-EmptyMaze.png");
        loadPinkBallsSystem_HeadOnly("datalcp/SimplePinky-EmptyMaze.png");
    ...
    /** Loads the TwoBlocks system */
    private void loadTwoBlocksSystem(String filename) { 
    	factory.use = false;
        twoBlocks.use = false;        
        systemClear();
        system.name = "Two Blocks";
        ImageBlocker blocker = new ImageBlocker( filename, (float) (double) whiteEpsilon.getValue() );
        imageWidth = blocker.width;
        imageHeight= blocker.height;
        system.bodies.addAll(blocker.bodies);
        ...
        //system.bodies.get(2).v.add(new Vector2d(10,10));
        Vector2d anchor1 = new Vector2d(system.bodies.get(1).x);
        Vector2d anchor2 = new Vector2d(system.bodies.get(2).x);

        //system.addJoint(j);
        for (RigidBody body :system.bodies)
        {
        	int myIndex = body.index;
        	double pos_x = body.x.x;
        	double pos_y = body.x.y;
        	//System.out.println("Body " + myIndex + "  x:[" + pos_x + "]  " +"y:[" + pos_y + "]");
        	//System.out.println("\t Mass: " + body.massLinear);
        }
        
        //System.out.println(j.toString());
        Vector2d b1c = new Vector2d(system.bodies.get(1).x.x, system.bodies.get(1).x.y);
        Vector2d b2c = new Vector2d(system.bodies.get(2).x.x, system.bodies.get(2).x.y);
        Vector2d temp = new Vector2d();
        temp.sub(b1c, b2c);
        
        
        //system.addConstraint(new DistanceConstraint(system.bodies.get(1), system.bodies.get(2), temp.length()));
        
    }
    
    /** Loads the Pinkballs system */
    private void loadPinkBallsSystem(String filename) { 
    	factory.use = false;
        twoBlocks.use = false;
        
        systemClear();
        system.name = "Pink Balls";
        ImageBlocker blocker = new ImageBlocker( filename, (float) (double) whiteEpsilon.getValue() );
        imageWidth = blocker.width;
        imageHeight= blocker.height;
        system.bodies.addAll(blocker.bodies);
        ...
        for (int i = 1; i < system.bodies.size()-1; i++)
        {
        	RigidBody body1 = system.bodies.get(i);
        	RigidBody body2 = system.bodies.get(i+1);
        	
        	body2.massLinear  *=0.6; 
        	body2.massAngular *=0.6;
        	
        	Spring s = new Spring(body1, body2);
        	system.springs.add(s);

        }
        
        //add controllers for each body
        for(int i = 0; i< system.bodies.size(); i++){
        	system.controllers.add(i, new Controller(system.bodies.get(i)));
        }
        
        //Adding magnet for head and tail
        RigidBody head = system.bodies.get(1);
        RigidBody tail = system.bodies.get(system.bodies.size()-1);
        
        system.magnets.add(new Magnet(head,tail));
        
        system.QLA = new QLearningAgent(system.controllers.get(1));   
    }
    
    /** Loads the Pinkballs system */
    private void loadPinkBallsSystem_HeadOnly(String filename) { 
    	factory.use = false;
        twoBlocks.use = false;
        
        systemClear();
        system.name = "Pink Balls_HeadOnly";
        ImageBlocker blocker = new ImageBlocker( filename, (float) (double) whiteEpsilon.getValue() );
        imageWidth = blocker.width;
        imageHeight= blocker.height;
        system.bodies.addAll(blocker.bodies);
        ...
        system.QLA_HeadOnly = new QLearningAgent_HeadOnly(system.controllers.get(1));   
    }
        ...
        component.addKeyListener( new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                if ( e.getKeyCode() == KeyEvent.VK_SPACE ) {
                	...
                	//Load Two blocks
                	loadTwoBlocksSystem("datalcp/test.png");
                	...
                	system.controllers.get(1).applyForceLeft(2000);
                	system.controllers.get(1).getInfo();
                	
                } else if ( e.getKeyCode() == KeyEvent.VK_RIGHT ) {
                	system.controllers.get(1).applyForceRight(2000);
                	system.controllers.get(1).getInfo();
                	
                } else if ( e.getKeyCode() == KeyEvent.VK_UP ) {
                	system.controllers.get(1).applyForceUp(2000);
                	system.controllers.get(1).getInfo();
                	
                } else if ( e.getKeyCode() == KeyEvent.VK_DOWN ) {
                	system.controllers.get(1).applyForceDown(2000); 
                	system.controllers.get(1).getInfo();
